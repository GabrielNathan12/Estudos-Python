...
class ButtonsGrid(QGridLayout):
    def __init__(
        ...
        self.window = window
        self._equation = ''
        self._equationInitialValue = 'Conta: '
        self._left = None
        self._right = None
        self._op = None
    ...
    def _makeGrid(self):
        ...
        for i, row in enumerate(self._gridMask):
            for j, btn in enumerate(row):
                ...
                if not isNumOrDot(btn) and not isEmpty(btn):
                    button.setProperty('cssClass', 'specialButton')
                    self._configSpecialButton(button)

                self.addWidget(button, i, j)
                slot = self._makeSlot(self._insertToDisplay, btn)
    ...
    def _eq(self):
        ...
        if not isValidNumber(displayText) or self._left is None:
            self._showError('Invalid account.')
            return
